...
async function onRequest(context2) {
  ...
  try {
    ...
    if (provider === "openrouter") {
      const openRouterApiKey = typeof apiKey === "string" && apiKey.trim() ? apiKey.trim() : env2.OPENROUTER_API_KEY;
      ...
      if (!response3.ok) return json({ success: false, message: data2.error?.message || `OpenRouter HTTP ${response3.status}` }, response3.status);
      return json({ success: true, content: data2.choices?.[0]?.message?.content || "" });
    }
    const geminiApiKey = typeof apiKey === "string" && apiKey.trim() ? apiKey.trim() : env2.GEMINI_API_KEY;
    ...
    const response2 = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${geminiModel}:generateContent?key=${geminiApiKey}`, {
      method: "POST",
    ...
    if (!response2.ok) return json({ success: false, message: data.error?.message || `Gemini HTTP ${response2.status}` }, response2.status);
    return json({ success: true, content: data.candidates?.[0]?.content?.parts?.[0]?.text || "" });
  } catch (error3) {
    return json({ success: false, message: error3 instanceof Error ? error3.message : "AI request gagal." }, 500);
  }
}
...
async function onRequest2(context2) {
  ...
  try {
    ...
    const apiKey = typeof payload.apiKey === "string" && payload.apiKey.trim() ? payload.apiKey.trim() : env2 && env2.RESEND_API_KEY || "";
    if (!apiKey) {
      return new Response(
...
async function onRequest3(context2) {
  ...
  if (request.method === "GET") {
    const settings = await env2.SETTINGS_KV.get(key, "json");
  ...
  if (request.method === "PUT") {
    try {
      ...
      await env2.SETTINGS_KV.put(key, JSON.stringify(settings));
      return response({ success: true });
    } catch (error3) {
      return response({ success: false, message: error3 instanceof Error ? error3.message : "Gagal menyimpan settings." }, 400);
    }
  }
...
// ../.wrangler/tmp/pages-gpW0f2/functionsRoutes-0.3466436356783581.mjs
var routes = [
  {
    routePath: "/api/ai",
    mountPath: "/api",
  ...
  async fetch(originalRequest, env2, workerContext) {
    ...
    const next = /* @__PURE__ */ __name(async (input, init) => {
      ...
      if (result.done === false) {
        ...
        const response2 = await handler(context2);
        if (!(response2 instanceof Response)) {
          throw new Error("Your Pages function should return a Response");
        }
        return cloneResponse(response2);
      } else if ("ASSETS") {
        const response2 = await env2["ASSETS"].fetch(request);
        return cloneResponse(response2);
      } else {
