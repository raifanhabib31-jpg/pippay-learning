  ...
  const getTypeBadge = (t: JadwalType) => {
    switch (t) {
      ...
      default:
        return {
          label: t,
          style: 'bg-dark-800 text-slate-300 border-dark-border',
        };
    }
  };
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          ...
          className="px-4 py-2.5 bg-accent-orange hover:bg-accent-orangeHover text-white rounded-xl text-xs font-bold flex items-center justify-center gap-2 shadow-lg shadow-orange-950/40 transition-all shrink-0"
        >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div 
          ...
          className="p-3 bg-dark-850 hover:bg-dark-800 border border-dark-border rounded-xl cursor-pointer transition-colors"
        >
          <div className="text-[10px] font-bold uppercase text-slate-400">Total Semua Agenda</div>
          <div className="text-xl font-bold text-white mt-1">{jadwalList.length}</div>
        </div>
        <div 
          onClick={() => setFilterType('lomba')}
          className="p-3 bg-dark-850 hover:bg-dark-800 border border-dark-border hover:border-amber-500/40 rounded-xl cursor-pointer transition-colors"
        >
          <div className="text-[10px] font-bold uppercase text-amber-400 flex items-center gap-1">
            <Trophy className="w-3 h-3" />
            <span>Lomba & Kompetisi</span>
          </div>
          <div className="text-xl font-bold text-amber-300 mt-1">{countLomba}</div>
        </div>
        <div 
          onClick={() => setFilterType('organisasi')}
          className="p-3 bg-dark-850 hover:bg-dark-800 border border-dark-border hover:border-purple-500/40 rounded-xl cursor-pointer transition-colors"
        >
          <div className="text-[10px] font-bold uppercase text-purple-400 flex items-center gap-1">
            <Users className="w-3 h-3" />
            <span>Organisasi & BEM</span>
          </div>
          <div className="text-xl font-bold text-purple-300 mt-1">{countOrg}</div>
        </div>
        <div 
          onClick={() => setFilterType('ujian')}
          className="p-3 bg-dark-850 hover:bg-dark-800 border border-dark-border hover:border-rose-500/40 rounded-xl cursor-pointer transition-colors"
        >
          <div className="text-[10px] font-bold uppercase text-rose-400">Ujian & UTS/UAS</div>
          <div className="text-xl font-bold text-rose-300 mt-1">{countUjian}</div>
        </div>
      </div>

      {/* Filter Tabs - NO EMOJIS */}
      <div className="flex p-1 bg-dark-850 rounded-xl border border-dark-border overflow-x-auto gap-1">
        {[
      ...
      {filteredJadwal.length === 0 ? (
        <div className="border border-dashed border-dark-border/80 rounded-3xl p-12 text-center space-y-4 bg-dark-900/40">
          ...
          <div className="space-y-1">
            <h3 className="text-base font-bold text-white">Belum Ada Agenda pada Kategori Ini</h3>
            <p className="text-xs text-slate-400">Tambahkan agenda ujian, rapat organisasi, atau jadwal lomba.</p>
          </div>
          <button
            onClick={() => setIsAddModalOpen(true)}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredJadwal.map((j) => {
            ...
            return (
              <div
                key={j.id}
                className={`bg-dark-850 hover:bg-dark-800 border rounded-2xl p-5 transition-all flex flex-col justify-between shadow-sm space-y-4 ${
                  hasReschedule
                    ? 'border-amber-500/50'
              ...
              >
                <div className="space-y-2.5">
                  <div className="flex items-center justify-between gap-2">
                    <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-md border ${badge.style}`}>
                      {badge.label}
                    </span>
                  ...
                  {/* Recurring badge */}
                  {j.isRecurring && (
                    <div className="flex items-center gap-1.5 text-[10px] text-blue-300 bg-blue-500/10 border border-blue-500/20 px-2 py-1 rounded-lg w-fit">
                  ...
                  {/* Reschedule notice */}
                  {hasReschedule && (
                    <div className="flex items-center justify-between gap-2 text-[10px] text-amber-300 bg-amber-500/10 border border-amber-500/20 px-2 py-1.5 rounded-lg">
                  ...
                  {hasReschedule && j.rescheduledForWeek?.reason && (
                    <p className="text-[10px] text-amber-400/70 italic pl-1">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-purple-300 font-semibold mt-0.5">{j.courseName}</p>
                  </div>
                  ...
                  <div className="space-y-1.5 text-xs text-slate-400">
                    ...
                    {j.notes && (
                      <p className="text-[11px] text-slate-400 line-clamp-2 mt-1 bg-dark-900 p-2 rounded-lg border border-dark-border/80">
                        {j.notes}
                      </p>
                    )}
                  </div>
                ...
                <div className="pt-3 border-t border-dark-border flex items-center justify-between gap-2">
                  <div className="flex items-center gap-1.5">
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-colors ${
                        j.reminderSent
                          ? 'bg-dark-800 text-emerald-400 hover:bg-dark-750'
                    ...
                    {/* Reschedule button - for recurring or kuliah type */}
                    {(j.isRecurring || j.type === 'kuliah') && (
                      <button
                        onClick={() => openRescheduleModal(j)}
                        className="px-3 py-1.5 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-colors bg-amber-600/20 text-amber-300 hover:bg-amber-600/30 border border-amber-500/30"
                  ...
                  <button
                    ...
                    className="p-1.5 text-slate-400 hover:text-rose-400 hover:bg-dark-750 rounded-lg transition-colors"
                    title="Hapus"
                  >
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 bg-black/75 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-dark-850 border border-dark-border rounded-2xl max-w-lg w-full p-6 text-slate-100 shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-dark-border pb-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="font-bold text-base text-white">Tambah Jadwal & Reminder</h3>
                  <p className="text-[11px] text-slate-400">Kuliah, Organisasi, Lomba, atau Kegiatan Luar</p>
                </div>
              </div>
              <button
            ...
            <form onSubmit={handleCreateJadwal} className="space-y-4 text-xs">
              <div>
                <label className="block text-slate-300 font-semibold mb-1">Judul Agenda *</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-dark-800 border border-dark-border rounded-xl px-3 py-2 text-xs text-slate-200 focus:outline-none focus:border-accent-orange"
                  >
                ...
                <div>
                  <label className="block text-slate-300 font-semibold mb-1">Nama Organisasi / Mata Kuliah / Event</label>
                  <input
                    type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-slate-300 font-semibold mb-1">Waktu (Jam)</label>
                  <input
                    type="time"
                    value={time}
              ...
              <div className="p-3 bg-dark-900 rounded-xl border border-dark-border space-y-3">
                <label className="flex items-center gap-3 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={isRecurring}
                    onChange={(e) => setIsRecurring(e.target.checked)}
                    className="w-4 h-4 rounded border-dark-border bg-dark-800 accent-purple-600"
                  />
                  <div className="flex items-center gap-1.5">
                    <Repeat className="w-3.5 h-3.5 text-blue-400" />
                    <span className="text-slate-200 font-semibold">Jadwal Rutin Mingguan</span>
                  </div>
                </label>
                ...
                {isRecurring && (
                  <div className="pl-7">
                    <label className="block text-slate-300 font-semibold mb-1">Hari Rutin</label>
                    <select
                      value={dayOfWeek}
                      onChange={(e) => setDayOfWeek(e.target.value)}
                      className="w-full bg-dark-800 border border-dark-border rounded-xl px-3 py-2 text-xs text-slate-200 focus:outline-none focus:border-blue-500"
                    >
              ...
              <div>
                <label className="block text-slate-300 font-semibold mb-1">Lokasi / Link Pendaftaran / Zoom</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-slate-300 font-semibold mb-1">Catatan Tambahan & Persiapan</label>
                <textarea
                  rows={2}
                  placeholder="Contoh: Bawa draft proposal, materi presentasi 10 slide, kartu identitas"
                  value={notes}
                  onChange={(e) => setNotes(e.target.value)}
                  className="w-full bg-dark-800 border border-dark-border rounded-xl p-2.5 text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-accent-orange"
                />
              </div>

              <div>
                <label className="block text-slate-300 font-semibold mb-1">Email Penerima Pengingat</label>
                <input
                  type="email"
          ...
          <div className="bg-dark-850 border border-dark-border rounded-2xl max-w-md w-full p-6 text-slate-100 shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-dark-border pb-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="font-bold text-base text-white">Ganti Jadwal Khusus Minggu Ini</h3>
                  <p className="text-[11px] text-slate-400">Perubahan hanya berlaku untuk minggu ini saja</p>
                </div>
              </div>
              <button
            ...
            <div className="p-3 bg-dark-900 rounded-xl border border-dark-border space-y-1 text-xs">
              <div className="text-[10px] font-bold uppercase text-slate-400">Jadwal Asli</div>
              <div className="text-white font-semibold">{rescheduleModal.jadwal.title}</div>
            ...
            <div className="space-y-3 text-xs">
              <div>
                <label className="block text-slate-300 font-semibold mb-1">Tanggal Pengganti *</label>
                <input
                  type="date"
                  value={rescheduleDate}
                  onChange={(e) => setRescheduleDate(e.target.value)}
                  className="w-full bg-dark-800 border border-dark-border rounded-xl px-3 py-2.5 text-xs text-slate-200 focus:outline-none focus:border-amber-500"
                  required
                />
              </div>

              <div>
                <label className="block text-slate-300 font-semibold mb-1">Waktu Pengganti</label>
                <input
                  type="time"
              ...
              <div>
                <label className="block text-slate-300 font-semibold mb-1">Alasan Penggantian</label>
                <textarea
                  rows={2}
                  placeholder="Contoh: Dosen tidak dapat mengajar hari ini, kelas dipindahkan ke hari Rabu"
                  value={rescheduleReason}
                  onChange={(e) => setRescheduleReason(e.target.value)}
                  className="w-full bg-dark-800 border border-dark-border rounded-xl p-2.5 text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-amber-500"
                />
              </div>
            </div>
            ...
            <div className="flex justify-end gap-2 pt-2 border-t border-dark-border">
              <button
                type="button"
                onClick={() => setRescheduleModal({ isOpen: false, jadwal: null })}
                className="px-4 py-2 bg-dark-800 hover:bg-dark-750 text-slate-300 rounded-xl font-semibold text-xs transition-colors"
              >
                Batal
              </button>
              <button
          ...
          <div className="bg-dark-850 border border-dark-border rounded-2xl max-w-lg w-full p-6 text-slate-100 shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-dark-border pb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-base text-white">Notifikasi Pengingat Terkirim</h3>
              </div>
              <button
            ...
            {emailResultModal.result.previewContent && (
              <div className="space-y-2 border border-dark-border rounded-xl p-3 bg-dark-900">
                <div className="text-[10px] font-bold text-slate-400 uppercase">Preview Format Email:</div>
                <div className="text-xs font-bold text-white">
                  Subjek: <span className="font-normal text-slate-300">{emailResultModal.result.previewContent.subject}</span>
                </div>
                <div className="p-3 bg-dark-950 border border-dark-border rounded-lg text-xs text-slate-300 whitespace-pre-line font-mono max-h-48 overflow-y-auto">
