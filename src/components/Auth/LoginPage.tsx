...
export const LoginPage: React.FC<LoginPageProps> = ({
  ...
  const slides = [
    {
      ...
      bgGradient: "from-amber-200 via-rose-300 to-indigo-600 dark:from-[#2a1b3d] dark:via-[#1e1530] dark:to-[#0f0c1b]",
      timeOfDay: "Dusk Chill"
    },
    {
      quote: "Smart Dual-Agent AI with Gemini & Kimi.",
      highlight: "Rangkum & Prediksi Ujian",
      bgGradient: "from-purple-300 via-indigo-400 to-slate-800 dark:from-[#1b2a4a] dark:via-[#15203b] dark:to-[#0c1224]",
      timeOfDay: "Night Focus"
    },
    {
      quote: "Stay ahead with automated H-1 exam reminders.",
      highlight: "IPK Maksimal & Terjadwal",
      bgGradient: "from-rose-200 via-purple-300 to-slate-900 dark:from-[#351b2e] dark:via-[#221323] dark:to-[#0f0c1b]",
  ...
  // Handle direct sign-in form submit -> redirect to Google Auth
  const handleDirectSignIn = async (e: React.FormEvent) => {
    e.preventDefault();
    if (emailInput.trim()) {
      setIsLoading(true);
      try {
        const user = await authService.loginWithGoogleDirect(emailInput.trim());
        onLoginSuccess(user);
      } catch (err: any) {
      ...
      // If empty, open the Google selection modal directly
      setIsGoogleModalOpen(true);
    }
  };

  // Submit Google simulation in modal
    ...
    <div className="min-h-screen w-full flex items-center justify-center p-4 sm:p-6 md:p-10 relative overflow-hidden bg-gradient-to-br from-[#c99587] via-[#a8746b] to-[#734b5f] dark:from-[#140f21] dark:via-[#0e0a19] dark:to-[#08060f] transition-colors duration-300 select-none">
      ...
      <div className="relative z-10 w-full max-w-4xl bg-[#f8f9fc] dark:bg-dark-900 border border-white/50 dark:border-dark-border rounded-[36px] shadow-2xl p-4 sm:p-5 md:p-6 grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
        ...
        <div className="px-3 sm:px-6 md:px-8 py-4 sm:py-6 flex flex-col justify-center space-y-6">
          ...
          {/* Single Google Login Button (Only Google as requested) */}
        ...
        <div className="relative h-80 sm:h-96 md:h-[480px] w-full rounded-[28px] overflow-hidden shadow-inner flex flex-col justify-between p-6 sm:p-8 bg-gradient-to-b from-[#e3b5a4] via-[#b6899e] to-[#4a3b63] dark:from-[#3a204f] dark:via-[#21153a] dark:to-[#0f091f]">
          ...
          <svg className="absolute inset-0 w-full h-full object-cover pointer-events-none" viewBox="0 0 400 500" fill="none" preserveAspectRatio="none">
            ...
            {/* Sun rays & abstract plant decor on ridge */}
      ...
      {/* ================= GOOGLE LOGIN MODAL (As shown in screenshot 2) ================= */}
      {isGoogleModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-[#181326] dark:bg-dark-900 border border-purple-900/40 dark:border-dark-border rounded-3xl p-6 sm:p-7 max-w-md w-full space-y-5 shadow-2xl text-slate-100 animate-in fade-in zoom-in-95 duration-150">
            ...
            <div className="space-y-2.5">
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-full bg-amber-500 text-white font-bold text-sm flex items-center justify-center shadow-xs">
                    A
                  </div>
                  <div>
                    ...
                    <div className="text-[11px] text-slate-400">alex.pratama@ui.ac.id</div>
                  </div>
                </div>
            ...
            <form onSubmit={handleGoogleSubmit} className="space-y-2 pt-3 border-t border-purple-900/30">
